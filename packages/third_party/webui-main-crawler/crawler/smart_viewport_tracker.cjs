...
const EVENTS_PER_VIEWPORT = 4; // Reduced for safer interactions
const HEADLESS = true; // Set to false to watch the browser
const VIEWPORT_HEIGHT = 800; // Fixed viewport height for consistent screenshots

function randomDelay(min = 1000, max = 2500) {
  // Shorter delays for more responsive interactions
...
async function simulateSession(sessionId, url) {
  ...
  for (let i = 0; i < numScreens; i++) {
    ...
    // Simulate SAFE user actions for this viewport (NO NAVIGATION)
    for (let j = 0; j < EVENTS_PER_VIEWPORT; j++) {
      try {
        // Only interact with elements that WON'T cause navigation
        const safeInteraction = await page.evaluate(() => {
          // Find safe elements: buttons, inputs, divs that don't navigate
          const safeElements = Array.from(document.querySelectorAll('button, input[type="button"], input[type="submit"], div[role="button"], span[role="button"]'))
            .filter(el => {
              ...
              return el.offsetParent !== null && 
                     ...
                     !id.includes('login') &&
                     !id.includes('signup') &&
                     !href.includes('http') && // No external links
                     !el.onclick && // No onclick handlers
                     !el.getAttribute('data-testid')?.includes('link') && // No test links
        ...
        // Random scroll within viewport (safe)
        await page.evaluate(() => window.scrollBy(0, Math.random() * 200 - 100));
        
        // Random input in safe fields (avoid login fields)
        await page.evaluate(() => {
