...
def label_points(A, B):
    ...
    labA = Text("A", font_size=32, color=BLACK).next_to(dotA, UP, buff=0.7)
...
class CorrespondingAnglesEqual(Scene):
    def construct(self):
        ...
        self.add(L1, L2, T)
        dotA = Dot(A, color=GRAY_E)
        dotB = Dot(B, color=GRAY_E)
        labA = Text("A", font_size=32, color=BLACK).next_to(dotA, UP, buff=0.7)
        labB = Text("B", font_size=32, color=BLACK).next_to(dotB, DOWN, buff=0.7)
        self.add(dotA, dotB, labA, labB)
        add_line_labels(self, L1, L2, T)
        leftL = -tL; upT = tT
        ...
        self.add(mid_tick(A, leftL, upT, radius=0.55, color=BLUE_B, count=2))
        self.add(mid_tick(B, leftL, upT, radius=0.55, color=BLUE_B, count=2))
